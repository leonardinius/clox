    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_CLASS: {
                ...
                Value initializer;
                if (tableGet((Table *)&klass->methods,  //
                             vm.initString, &initializer)) {
...
static bool invokeFromClass(ObjClass *klass, ObjString *name, int argCount) {
    Value method;
    if (!tableGet((Table *)&klass->methods, name, &method)) {
...
static bool invoke(ObjString *name, int argCount) {
    ...
    Value value;
    if (tableGet((Table *)&instance->fields, name, &value)) {
...
static ObjUpvalue *captureUpvalue(Value *local) {
    ...
    createdUpvalue->next = (void *)upvalue;
    if (prevUpvalue == NULL) {
        vm.openUpvalues = createdUpvalue;
    } else {
        prevUpvalue->next = (void *)createdUpvalue;
    }
...
static void defineMethod(ObjString *name) {
    ...
    tableSet((Table *)&klass->methods, name, method);
    pop();
}
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_PROPERTY: {
                ...
                tableSet((Table *)&instance->fields, READ_STRING(), peek(0));
                Value value = pop();
                pop();
                push(value);
                break;
            ...
            case OP_INHERIT: {
                ...
                tableAddAll((Table *)&AS_CLASS(superclass)->methods,
                            (Table *)&subclass->methods);
                pop();  // Subclass.
                break;
            }
